...
func verifyCreemSignature(payload string, signature string, secret string) bool {
	if secret == "" {
		logger.LogWarn(context.Background(), fmt.Sprintf("Creem webhook secret 未配置 test_mode=%t signature=%s body=%s", setting.CreemTestMode, common.SafeLogSecret(signature), common.SafeLogPayload([]byte(payload))))
		if setting.CreemTestMode {
			logger.LogInfo(context.Background(), fmt.Sprintf("Creem webhook 验签已跳过 reason=test_mode signature=%s body=%s", common.SafeLogSecret(signature), common.SafeLogPayload([]byte(payload))))
			return true
		}
		return false
	}
...
func RequestCreemPay(c *gin.Context) {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem 支付请求已收到 user_id=%d body=%s", c.GetInt("id"), common.SafeLogPayload(bodyBytes)))
...
func CreemWebhook(c *gin.Context) {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem webhook 收到请求 path=%q client_ip=%s signature=%s body=%s", c.Request.RequestURI, c.ClientIP(), common.SafeLogSecret(signature), common.SafeLogPayload(bodyBytes)))
	if signature == "" {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Creem webhook 缺少签名 path=%q client_ip=%s body=%s", c.Request.RequestURI, c.ClientIP(), common.SafeLogPayload(bodyBytes)))
	...
	if !verifyCreemSignature(string(bodyBytes), signature, setting.CreemWebhookSecret) {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Creem webhook 验签失败 path=%q client_ip=%s signature=%s body=%s", c.Request.RequestURI, c.ClientIP(), common.SafeLogSecret(signature), common.SafeLogPayload(bodyBytes)))
		c.AbortWithStatus(http.StatusUnauthorized)
		return
	}
	...
	if err := c.ShouldBindJSON(&webhookEvent); err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Creem webhook 解析失败 path=%q client_ip=%s error=%q body=%s", c.Request.RequestURI, c.ClientIP(), err.Error(), common.SafeLogPayload(bodyBytes)))
		c.AbortWithStatus(http.StatusBadRequest)
		return
	}
...
func genCreemLink(ctx context.Context, referenceId string, product *CreemProduct, email string, username string) (string, error) {
	...
	logger.LogInfo(ctx, fmt.Sprintf("Creem API 响应已收到 trade_no=%s status_code=%d body=%s", referenceId, resp.StatusCode, common.SafeLogPayload(body)))
